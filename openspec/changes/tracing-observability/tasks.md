...
- [ ] 1.1 Create `app/tracing/` package and `tests/tracing/` package.
- [ ] 1.2 Author `tasks.md` and `loop.md` (this file + loop state). First commit.

## 2. PII mask (`app/tracing/mask.py`)

- [ ] 2.1 `mask_pii(data: Any) -> Any` conforming to the frozen `MaskFn` signature.
      Recurses `dict`/`list`/`str`. Reuses the frozen `PII_KEYS` for key-based
      redaction (names + credentials + amounts by key).
- [ ] 2.2 Value-level regex redaction on strings, per the proposal's concrete list:
...
- [ ] 2.3 Tests: every PII class redacted in nested inputs/outputs; non-PII intact.

## 3. Span helpers + thread stitching + manual llm-span (`app/tracing/spans.py`)

- [ ] 3.1 Typed span helpers `agent_span`/`retriever_span`/`llm_span`/`tool_span`
      (= `observe(type=...)`); re-export `observe`, `update_current_span`,
      `update_current_trace`.
- [ ] 3.2 `stitch_thread(thread_id, user_id, *, turn_number, model_version)` wrapping
      `update_current_trace(...)` for the §6.4 multi-turn stitch.
- [ ] 3.3 `log_llm_span(response, model)` — manual Path B: reads the Anthropic
      response `usage` (`input_tokens`/`output_tokens`) onto the current `llm` span
      via `update_llm_span()`.
- [ ] 3.4 Tests: helpers carry the right `type`; stitch_thread sets
...
- [ ] 4.1 `configure_tracing(environment, sampling_rate=None, confident_api_key=None)`
...
- [ ] 4.2 Anthropic auto-patch probe: Path A (configure accepts `anthropic_client`) →
      forward the pinned client; Path B (only `openai_client`) → fall back to manual
      `log_llm_span`. Log which is active.
- [ ] 4.3 `maybe_clear_traces()` — periodic `trace_manager.clear_traces()` for the
      long-running server (only when not exporting to Confident AI).
- [ ] 4.4 `assert_no_local_metrics(environment, metrics=None)` — refuses local
      LLM-judge `metrics=[...]` in production (§6.5); reuses frozen
      `inline_judge_allowed`.
- [ ] 4.5 Tests: both paths exercised via a stubbed DeepEval `configure`; prod guard
      raises; sampling defaults per environment; maybe_clear_traces clears.

## 5. Public API + gates

- [ ] 5.1 `app/tracing/__init__.py` re-exports the public surface.
- [ ] 5.2 `pytest tests/tracing/` green; doneCondition passes; offline (no Confident
      AI connection, no real LLM call).

## 6. Verify + ship

- [ ] 6.1 Fresh spec-verifier panel (3 lenses). Fix divergences; re-panel if blocking.
- [ ] 6.2 Rebase onto latest origin/main; full `uv run pytest` green; push; open PR.
