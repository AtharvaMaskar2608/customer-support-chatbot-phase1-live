...
- [x] Create `app/ticketing/` package (`__init__.py` tool-registration surface).
- [x] `app/ticketing/freshdesk.yaml` — the 04 §5 config: group_id/source/status/
...
- [x] `app/ticketing/config.py` — typed loader for freshdesk.yaml + `${ENV}` refs.

## 2. Query-type → Freshdesk field mapping
- [x] `app/ticketing/mapping.py` — `freshdesk_type_for_intent`, `subject_sub_type_for_intent`,
      status enum → user copy (2 Open / 3 Pending / 4 Resolved / 5 Closed).

## 3. Payload + transcript/metadata assembly
- [x] `app/ticketing/payload.py` — `TranscriptTurn` type; HTML-escaped transcript
...
- [x] `app/ticketing/client.py` — httpx.AsyncClient wrapper; Basic auth
...
- [x] `app/ticketing/tool.py` — `raise_ticket`: session-scoped idempotency guard →
...
- [x] `tests/ticketing/` — test_payload, test_mapping, test_dedupe, test_status_lookup,
      test_errors + recorded fixtures (201 create, list, search, view+stats, 400/409/429).

## Done
- [x] `pytest tests/ticketing/` green; doneCondition satisfied; fresh verifier panel clean.
