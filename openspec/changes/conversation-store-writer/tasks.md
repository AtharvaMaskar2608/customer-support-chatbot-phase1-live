...
- [x] Author this task list and the loop.md loop-state file, recording the
      prose↔frozen-contract reconciliation decisions (see loop.md "Contract
      reconciliation").

## 2. Migration 0002 — write-side idempotency guard
- [x] `app/store/migrations/0002_turns_idempotency_guard.sql`: additive,
...
- [x] `ConversationStoreWriter`: bounded `asyncio.Queue` + single background
      worker with its own DB connection (from the consumed connection factory).
- [x] `enqueue(record)` — non-blocking `put_nowait`; on `QueueFull` → log +
      `store_write_dropped` metric + drop (identifiers only, never message PII);
      never raises into the caller.
- [x] `start()` / `stop()` — spawn the worker; drain-then-cancel on stop. These
      are the lifespan hooks main.py (orchestrator-owned) calls.
- [x] `_worker()` — `await queue.get()` → `_insert_turn`; any exception →
      log + metric + drop, continue; `task_done()` each item.
- [x] `_insert_turn(record)` — one transaction: upsert the parent `threads` row
...
## 4. Tests — `tests/store/`
- [x] Non-blocking ordering: `enqueue()` returns before the insert runs.
- [x] Full-column insert: every §3.2 column populated (threads upsert + turns
      insert), asserted against a fake async connection double.
- [x] Forced insert error → log-and-drop, worker survives, caller never sees it.
- [x] Queue-full → log-and-drop, `enqueue` never raises, metric increments.
- [x] `start()`/`stop()` drain queued records then shut down cleanly.
- [x] Migration: `0002` discovered after `0001`, applies on top (dry-run parse —
